//Express for Administration panel.
const usersRouter = require("express").Router();
const { request } = require("express");
// const Systems = require("../models/systems_db.js");
let {menuCacheAdminClientAdd, menuCacheAction} = require("../store/menu_store.js");
...
usersRouter.get("/:id", (request, response) => {
    ...
    if(!(menuClientId in menuObj.clientsCache)){
        menuCacheAdminClientAdd(menuClientId);
    }
    response.
        status(200).
...
usersRouter.post("/:id", (request, response) => {
    ...
    if(!(request.body.menu) || !(request.body.action) || !(request.body.menu)) 
        ...
        let reply = postActionAndReply(request.params.id,request.body,true) ? "ok" : "error";
...
function getReply(id){
    let returnData={
        ...
        menus:{
            ...
            names: menuObj.menuNames, //admins receives all menus
    ...
    menuObj.clientsCache[id].menuList.forEach((el)=>{
        if(global[el]) if(global[el].timeStamp) returnData.data[el]=global[el];
    })
